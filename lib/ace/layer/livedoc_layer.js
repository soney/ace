...
var Livedoc = function(parentEl) {
	this.under_text = dom.createElement("div");
    this.under_text.className = "ace_layer ace_livedoc-overlay";
	parentEl.insertBefore(this.under_text, parentEl.childNodes[0]);
...
(function() {
    ...
    this.update = function(config, changes) {
		this.config = config;
        if (config === undefined || this.model === undefined) {
            return;
		}
		this.model.update_annotations(this.config, changes);
    };
