...
(function() {
	...
	this.setModel = function(livedoc_model) {
		...
		if(this.model) {
			...
			this.model.removeEventListener("annotationChange", this.$updateWindowPosition);
		}
		...
		if(this.model) {
			...
			this.model.addEventListener("annotationChange", this.$updateWindowPosition);
		}
	};
	...
	this.getRowMarker = function(from_row, to_row, config, clazz) {
		...
		else if(from_row !== null && to_row === null) { //From somewhere to bottom of file
	...
	this.updateOverlay = function() {
		...
		overlay_str.push(this.getMarker(range.end.row, range.end.column, null, null, config, overlay_class));


		var outline = this.getOutline(range, config, {fill: "none", stroke: "black"});


		var highlights = this.model.get_highlights();
		var highlight_str = [];
		for(var i = 0, leni = highlights.length; i<leni; i++) {
			var highlight = highlights[i];
			var ranges = highlight.ranges,
				options = highlight.options;
			for(var j = 0, lenj = ranges.length; j<lenj; j++) {
				var range = ranges[j];
				highlight_str.push(this.getOutline(range, config, options));
			}
		}

		overlay_str.push("<svg>", outline, highlight_str, "</svg>");
	...
	this.updateWindowPosition = function() {
		...
		if(model.is_search()) {
			...
			var height_pixels = 100;
			var height_rows = Math.ceil(height_pixels/this.config.lineHeight);
			this.session.setRowspace(model.insertionRange.end.row, height_rows);

			var height = box.height();
			var line_top = this.$getTop(insertionStart.row+1, this.config);
	...
	this.getOutline = function(from_row, from_col, to_row, to_col, config, options) {
		...
		if(from_row === to_row) {
		   ...
		   */
			...
			return '<rect x="'+tl.x+'" y="'+tl.y+'" width="'+w+'" height="'+h+'" fill="'+fill+'" stroke="'+stroke+'" />';
		}
		else {
			if(to_row - from_row === 1 && to_col < from_col) {
				...
				var rv = '<path d="'+tr_str+' '+bl_str+'" style="stroke:'+stroke+'; fill:'+fill+';"/>';
			   ...
			   */
				...
				return '<polygon points="'+points_str+'" fill="'+fill+'" stroke="'+stroke+'" />';
			}
		}
		return "";
	};
