...
(function() {
    ...
    this.moveText = function(fromRange, toPosition) {
		...
		if(this.hasLiveDoc()) {
			var livedoc = this.getLiveDoc();
			livedoc.on_move_text(fromRange, toPosition, toRange);
		}
    ...
    this.$updateWrapData = function(firstRow, lastRow) {
        ...
        while (row <= lastRow) {
			...
			*/
			wrapLimit = this.$wrapLimit;
            wrapData[row] =
                this.$computeWrapSplits(tokens, wrapLimit, tabSize, row);
    ...
    this.$computeWrapSplits = function(tokens, wrapLimit, tabSize, row) {
        if (tokens.length == 0) {
            //return [];
        }
		...
		var rowSpaces = this.$rowSpaces[row];
		if(rowSpaces !== undefined) {
			var num_splits = rowSpaces;
	...
	this.setLivedoc = function(livedoc) {
		this.livedoc = livedoc;
		this._dispatchEvent("livedocChanged")
	};
	this.hasLivedoc = function() {
		return this.hasOwnProperty("livedoc");
	};
	this.getLivedoc = function() {
		return this.livedoc;
	};
	this.invokeLivedoc = function() {
		this.livedoc.invoke();
	};
	this.setRowspace = function(row, num_rows) {
		if(row instanceof Array) {
			var changed_min = Infinity;
			var changed_max = -1;
			var rows = row;

			for(var i = 0, len = rows.length; i<len; i++) {
				if(this.$rowSpaces[i] !== rows[i]) {
					changed_min = Math.min(i, changed_min);
					changed_max = Math.max(i, changed_min);
					this.$rowSpaces[i] = rows[i];
				}
			}

			if(changed_min>=0 && changed_max>=0) {
				this.$updateWrapData(changed_min, changed_max+1);
				this._dispatchEvent("changeWrapMode");
				return true;
			}
			return false;
		...
		if(this.$rowSpaces[row] !== num_rows) {
			this.$rowSpaces[row] = num_rows;
			this.$updateWrapData(row, row+1);
			this._dispatchEvent("changeWrapMode");
			return true;
		}
	};
	this.removeRowspace = function(row) {
		delete this.$rowSpaces[row];
		this.$updateWrapData(row, row+1);
		this._dispatchEvent("changeWrapMode");
	};
	/*
	this.setColsize = function(from_row, to_row, num_cols) {
		if(from_row instanceof Array) {
			var rows = from_row;
			var changed_min = Infinity
				, changed_max = -1;

			for(var row = 0, len = rows.length; row<len; row++) {
				if(this.$colSizes[row] !== rows[row]) {
					changed_min = Math.min(row, changed_min);
					changed_max = Math.max(row, changed_min);
					this.$colSizes[row] = rows[row];
				}
			}
		...
		var min = Math.min(from_row, to_row);
		var max = Math.max(from_row, to_row);
		for(var row = min; row<=max; row++) {
			this.$colSizes[row] = num_cols;
		}
		this.$updateWrapData(min, max+1);
		this._dispatchEvent("changeWrapMode");
	};
	this.unsetColsize = function(from_row, to_row) {
		if(arguments.length === 2) {
			to_row = from_row;
			num_cols = to_row;
		}
