...
(function() {
    ...
    this.$computeWrapSplits = function(tokens, wrapLimit, tabSize, row) {
		...
		if(rowSpaces !== undefined) {
			...
			for(var i = 0; i<num_splits; i++) {
				splits.push(9999999999999999999999);
			}
		}
	...
	this.setRowspace = function(row, num_pixels) {
		if(this.$rowSpaces[row] !== num_pixels) {
			this.$rowSpaces[row] = num_pixels;
			this.$updateWrapData(row);
			this._dispatchEvent("changeWrapMode");
		}
	};
