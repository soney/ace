...
var TextInput = function(parentNode, host) {
    ...
    var onCopy = function(e) {
		...
		if(session.hasLivedoc()) {
			var livedoc = session.getLivedoc();
			livedoc.on_copy(host.getSelectionRange(), copyText);
		}
        setTimeout(function () {
            sendText();
        }, 0);
    ...
    var onCut = function(e) {
        copied = true;
        var copyText = host.getCopyText();
		var selectionRange = host.getSelectionRange();
        if(copyText) {
            text.value = copyText;
            host.onCut();
        } else
		...
		if(session.hasLivedoc()) {
			var livedoc = session.getLivedoc();
			livedoc.on_copy(selectionRange, copyText);
		}
        text.select();
        setTimeout(function () {
            sendText();
    ...
    event.addListener(text, "paste", function(e) {
        ...
        if (e.clipboardData && e.clipboardData.getData) {
			...
			var session = host.getSession();
			var selection_pre_send = host.getSelectionRange();
            sendText(data);
            e.preventDefault();
			var selection_post_send = host.getSelectionRange();

			if(session.hasLivedoc()) {
				var livedoc = session.getLivedoc();
				livedoc.on_paste({start: selection_pre_send.start, end: selection_post_send.end}, data);
			}
        } else
